...
const STEPS = ['Client Details', 'Event Setup', 'Menu Selection', 'Review'];
...
const DECORATION_OPTIONS = ['basic', 'premium', 'luxury', 'custom'];
const ENTERTAINMENT_OPTIONS = ['none', 'dj', 'live-band', 'custom'];
...
export default function NewBooking() {
  ...
  const [form, setForm] = useState({
    ...
    eventManager: '', notes: '',
    decoration: 'basic', entertainment: 'none', photography: false,
    tier: '', addons: [],
  });
  ...
  useEffect(() => {
    if (!form.date || !form.venue) { setOverlap(null); return; }
    clearTimeout(debounceRef.current);
    setCheckingAvail(true);
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await api.checkAvailability(form.date, form.venue);
        setOverlap(res.available ? null : `${form.venue} is already booked on ${form.date} (${res.conflict?.enquiryId}).`);
      } catch {
        setOverlap(null);
      } finally {
  ...
  const canNext = () => {
    if (step === 0) return form.partyName && form.clientName && form.clientPhone && form.clientEmail && form.eventType;
    if (step === 1) return form.date && form.startTime && form.endTime && form.venue && form.pax && form.eventManager && !overlap && !checkingAvail;
  ...
  if (submitted) {
    const enquiryId = submitResult?.booking?.enquiryId || 'ENQ-NEW';
    const deadline = submitResult?.booking?.advanceDeadline
      ? new Date(submitResult.booking.advanceDeadline).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })
      : '48 hours from now';
    return (
      <div className="nb">
        <div className="nb__success">
          ...
          <p className="nb__success-sub">
            Booking <strong>{enquiryId}</strong> has been created as a <em>Temporary Enquiry</em>.
          </p>
          <div className="nb__alert nb__alert--info" style={{ maxWidth: 480, margin: '16px auto 0' }}>
            <span className="nb__alert-icon">⏰</span>
            <div>
              <strong>48-Hour Advance Payment Required</strong>
              <p>Pay 25% advance by <strong>{deadline}</strong> to confirm your spot. Unpaid slots are auto-released.</p>
            </div>
          </div>
          {submitError && <p style={{ color: '#E85555', marginTop: 8 }}>{submitError}</p>}
          <p style={{ color: '#9D9880', fontSize: 13, marginTop: 12 }}>A WhatsApp confirmation will be sent to {form.clientPhone}.</p>
          <div className="nb__success-actions">
            <button onClick={() => navigate('/sales')} className="nb__success-btn nb__success-btn--primary">
              Back to Dashboard
            </button>
            <button onClick={() => { setSubmitted(false); setStep(0); setSubmitResult(null); setForm({ partyName:'',clientName:'',clientPhone:'',clientEmail:'',gstNumber:'',companyName:'',alternatePhone:'',address:'',eventType:'',date:'',startTime:'',endTime:'',venue:'',pax:'',eventManager:'',notes:'',decoration:'basic',entertainment:'none',photography:false,tier:'',addons:[] }); }} className="nb__success-btn">
              New Booking
            </button>
          </div>
        </div>
          ...
          <div className="nb__panel fade-up">
            ...
            <div className="nb__grid-2">
              ...
              <Field label="Alternate Phone">
                <input className="nb__input" type="tel" placeholder="Optional"
              ...
              <Field label="Client Address" className="nb__field--full">
                <textarea className="nb__input nb__textarea" rows={2} placeholder="Full address for correspondence…"
